...
.logo-name { font-size: 17px; font-weight: 800; color: #fff; }
.logo-name span { color: #7c6fff; }
.logo-sub { font-size: 11px; color: #4b5563; margin-top: 1px; }
.tr { display: flex; align-items: center; gap: 20px; }
.live-pill {
...
.tr-time { text-align: right; }
.tr-time .t1 { font-size: 17px; font-weight: 700; color: #fff; font-variant-numeric: tabular-nums; }
.tr-time .t2 { font-size: 11px; color: #4b5563; margin-top: 1px; }
.tr-time .t3 { font-size: 12px; color: #6c63ff; font-weight: 600; }
...
.wl h2 { font-size: 22px; font-weight: 800; color: #fff; margin-bottom: 6px; }
.wl p { font-size: 13px; color: #6b7280; }
.features { display: flex; gap: 28px; margin-top: 14px; flex-wrap: wrap; }
...
.feat-text .ft { font-size: 12px; font-weight: 700; color: #e2e8f0; }
.feat-text .fs { font-size: 11px; color: #4b5563; }

/* ── Clock cards ── */
.clock-cards { display: flex; gap: 10px; }
.clock-card {
    background: #12131f;
...
.cc-top { display: flex; align-items: center; gap: 7px; margin-bottom: 6px; }
.cc-flag { font-size: 15px; }
.cc-zone { font-size: 11px; font-weight: 700; color: #4b5563; text-transform: uppercase; letter-spacing: 0.08em; }
.cc-time { font-size: 22px; font-weight: 700; color: #fff; line-height: 1; font-variant-numeric: tabular-nums; }
.cc-date { font-size: 11px; color: #4b5563; margin-top: 5px; }
...
.us-title { font-size: 15px; font-weight: 700; color: #fff; }
.us-badge {
...
.drop-zone:hover { border-color: #6c63ff; background: rgba(108,99,255,0.03); }
.drop-cloud { font-size: 44px; margin-bottom: 14px; display: block; }
.drop-t1 { font-size: 18px; font-weight: 700; color: #fff; margin-bottom: 4px; }
.drop-t2 { font-size: 13px; color: #6b7280; margin-bottom: 4px; }
.drop-t3 { font-size: 12px; color: #4b5563; }

/* Choose file button (Streamlit override) */
section[data-testid="stFileUploadDropzone"] {
    background: transparent !important;
    border: none !important;
    padding: 0 !important;
}
section[data-testid="stFileUploadDropzone"] > div:first-child {
...
.subf-left { display: flex; align-items: center; gap: 10px; }
.subf-icon {
    width: 32px; height: 32px; border-radius: 8px;
    background: rgba(108,99,255,0.1); border: 1px solid rgba(108,99,255,0.2);
    display: flex; align-items: center; justify-content: center; font-size: 15px;
}
.subf-text .st1 { font-size: 13px; font-weight: 600; color: #e2e8f0; }
.subf-text .st2 { font-size: 11px; color: #4b5563; margin-top: 1px; }
.subf-arrow { color: #374151; font-size: 18px; }
...
.sb-text .s1 { font-size: 13px; font-weight: 600; color: #e2e8f0; }
.sb-text .s2 { font-size: 11px; color: #4b5563; margin-top: 2px; }
...
.rb-title { font-size: 14px; font-weight: 700; color: #a78bfa; margin-bottom: 12px; text-align: center; }
.rb-stats { display: grid; grid-template-columns: repeat(4,1fr); gap: 8px; }
...
.rbs-l { font-size: 10px; color: #4b5563; text-transform: uppercase; letter-spacing: 0.06em; }
...
<div class="topbar">
  <div class="tl">
    ...
    <div>
      <div class="logo-name">MOA <span>Extractor</span></div>
      <div class="logo-sub">PO Count Sheet Extractor</div>
    </div>
  </div>
  <div class="tr">
    <div class="live-pill"></div>
    <div class="tr-time">
      <div class="t1" id="nav-time">--:--:-- --</div>
      <div class="t2" id="nav-date">--</div>
      <div class="t3">Hsenid</div>
    </div>
...
<div class="welcome-section">
  <div class="wl">
    ...
    <div class="features">
      <div class="feat">
        ...
        <div class="feat-text">
          <div class="ft">No steps</div>
          <div class="fs">Just upload your file</div>
        </div>
      </div>
      <div class="feat">
        <div class="feat-icon">💻</div>
        <div class="feat-text">
          <div class="ft">No code</div>
          <div class="fs">We handle the rest</div>
        </div>
      </div>
      <div class="feat">
        <div class="feat-icon">⚡</div>
        <div class="feat-text">
          <div class="ft">Instant Excel</div>
          <div class="fs">Download in seconds</div>
        </div>
      </div>
    </div>
  </div>
  <div class="clock-cards">
    <div class="clock-card">
      <div class="cc-top"><span class="cc-flag">🇮🇳</span><span class="cc-zone">IST</span></div>
      <div class="cc-time" id="ist-t">--:--:--</div>
      <div class="cc-date" id="ist-d">--</div>
    </div>
    <div class="clock-card">
      <div class="cc-top"><span class="cc-flag">🇫🇯</span><span class="cc-zone">FJT</span></div>
      <div class="cc-time" id="fjt-t">--:--:--</div>
      <div class="cc-date" id="fjt-d">--</div>
    </div>
    <div class="clock-card">
      <div class="cc-top"><span class="cc-flag">🇺🇸</span><span class="cc-zone">EST</span></div>
      <div class="cc-time" id="est-t">--:--:--</div>
      <div class="cc-date" id="est-d">--</div>
    </div>
  </div>
</div>
""", unsafe_allow_html=True)
...
<div class="upload-section">
  <div class="us-header">
    <div class="us-title">Upload PO PDF</div>
    <div class="us-badge">Max 200MB per file</div>
  </div>
  <div class="drop-zone">
    <span class="drop-cloud">☁️</span>
...
<div style="margin:14px 28px 0">
  <div class="subf-row">
    <div class="subf-left">
      <div class="subf-icon">#️⃣</div>
      <div class="subf-text">
        <div class="st1">Optional: SUBF number mapping</div>
        <div class="st2">Improve extraction accuracy</div>
      </div>
    </div>
    <span class="subf-arrow">›</span>
  </div>
</div>
""", unsafe_allow_html=True)
...
with st.expander("", expanded=False):
    subf_text = st.text_area(
        "SUBF", placeholder="SUBF 103566\t88251-24\nSUBF 103567\t88251-25",
        height=80, label_visibility="collapsed"
    )

st.markdown('<div style="margin:0 28px">', unsafe_allow_html=True)
...
<div class="security-bar">
  <div class="sb-left">
    ...
    <div class="sb-text">
      <div class="s1">Your data is secure</div>
      <div class="s2">We never store your files. All processing is done in real-time and files are automatically cleared.</div>
    </div>
  </div>
...
# ── CLOCK JS (runs in browser for correct local timezone) ──
components.html("""
<script>
...
function tick(){
  try{
    ...
    function set(id,val){var el=window.parent.document.getElementById(id);if(el)el.textContent=val;}
    set('ist-t',fT(ist)); set('ist-d',fD(ist));
    set('fjt-t',fT(fjt)); set('fjt-d',fD(fjt));
    set('est-t',fT(est)); set('est-d',fD(est));
    set('nav-time',fT(now)); set('nav-date',fD(now));
  }catch(e){}
}
...
if extract_btn and uploaded_files:
    st.markdown('<div style="margin:16px 28px 0">', unsafe_allow_html=True)
    ...
    if not all_rows_by_key:
        ...
        <div class="result-box">
          <div class="rb-title">✅ Excel Ready — {output_filename}</div>
          <div class="rb-stats">
