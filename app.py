...
SIZE_TOKENS = (
    ...
    '5 TO 8','9 TO 2','2 TO 7','8 TO 11','12 TO 14','14+',
    '5 TO 7','8 TO 10','11 TO 13','5 TO 9','9 TO 12','6 TO 10','10 TO 13',
    'One'
)
...
def merge_size_tokens(words):
    """Merge multi-word size tokens: TO N, N TO M, 14+ etc."""
    result = []
    i = 0
    while i < len(words):
        w = words[i]
        # Handle: N TO M  (sock sizes like 5 TO 8, 8 TO 11)
        if (w['text'].replace("+","").isdigit() or w['text'] == "14+") and i + 2 < len(words):
            if words[i+1]['text'].upper() == 'TO':
                combined = f"{w['text']} TO {words[i+2]['text']}"
                if combined in SIZE_TOKENS_SET:
                    merged = dict(w); merged['text'] = combined
                    result.append(merged); i += 3; continue
        # Handle: TO N  (existing)
        if w['text'].upper() == 'TO' and i + 1 < len(words):
            combined = f"TO {words[i+1]['text']}"
            if combined in SIZE_TOKENS_SET:
                merged = dict(w); merged['text'] = combined
                result.append(merged); i += 2; continue
        # Handle: 14+  (standalone)
        if w['text'] in SIZE_TOKENS_SET:
            result.append(w); i += 1; continue
        result.append(w); i += 1
    return result
...
.app-card{
    background:#0d0e1c;border:1px solid #1e2040;
...
/* TOPBAR */
.topbar{
    background:#13142a;border:1px solid #2a2b55;
...
.live-dot{width:10px;height:10px;background:#22c55e;border-radius:50%;animation:glow 2s infinite;flex-shrink:0;}
@keyframes glow{0%,100%{box-shadow:0 0 0 0 rgba(34,197,94,0.5)}50%{box-shadow:0 0 0 6px rgba(34,197,94,0)}}
.tr-right{text-align:right;}
.tr-time{font-size:18px;font-weight:700;color:#ffffff;font-variant-numeric:tabular-nums;line-height:1;}
.tr-date{font-size:11px;color:#6b7280;margin-top:3px;}
.tr-brand{font-size:13px;font-weight:700;color:#a78bfa;margin-top:2px;}

/* WELCOME ROW */
.welcome-row{display:flex;align-items:flex-start;justify-content:space-between;gap:20px;}
.wl{flex:1;min-width:0;}
.wl h2{font-size:22px;font-weight:800;color:#ffffff;margin-bottom:6px;}
.wl p{font-size:13px;color:#9ca3af;margin-bottom:14px;}
...
.ft{font-size:13px;font-weight:700;color:#e2e8f0;}
.fs{font-size:11px;color:#6b7280;}
.clocks{display:flex;gap:10px;flex-shrink:0;}
.cc{background:#13142a;border:1px solid #2a2b55;border-radius:12px;padding:12px 16px;min-width:148px;}
.cc-top{display:flex;align-items:center;gap:8px;margin-bottom:6px;}
/* SVG flag icons — consistent across all browsers */
.flag-svg{width:22px;height:16px;border-radius:3px;flex-shrink:0;}
.cc-z{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;}
.cc-t{font-size:20px;font-weight:700;color:#ffffff;font-variant-numeric:tabular-nums;line-height:1;}
.cc-d{font-size:11px;color:#6b7280;margin-top:5px;}
.ist .cc-z{color:#818cf8;}.fjt .cc-z{color:#a78bfa;}.est .cc-z{color:#fbbf24;}

/* UPLOAD — single clean drop zone */
.upload-section{background:#13142a;border:1px solid #2a2b55;border-radius:16px;padding:20px;}
.uc-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;}
.uc-title{font-size:15px;font-weight:700;color:#ffffff;}
.uc-badge{font-size:11px;color:#6b7280;background:#0d0e1c;border:1px solid #2a2b55;border-radius:6px;padding:4px 10px;}

/* Hide all Streamlit uploader default styling, keep only the functional part */
[data-testid="stFileUploadDropzone"]{
    background:#0d0e1c!important;
    border:2px dashed #3a3b6a!important;
...
/* Replace default icon and text with custom styled ones */
...
[data-testid="stFileUploaderDropzoneInstructions"] span{
    color:#9ca3af!important;font-size:13px!important;text-align:center!important;
}
...
/* Drop zone custom overlay */
.drop-overlay{
    text-align:center;pointer-events:none;
    display:flex;flex-direction:column;align-items:center;gap:6px;
    margin-bottom:16px;
}
.drop-t1{font-size:18px;font-weight:700;color:#ffffff;}
.drop-t2{font-size:13px;color:#9ca3af;}
.drop-t3{font-size:11px;color:#4b5563;}

/* File selected */
.file-selected{
    background:rgba(108,99,255,0.1);border:1px solid rgba(108,99,255,0.25);
    border-radius:10px;padding:10px 14px;font-size:12px;color:#a78bfa;margin-top:10px;
}
.file-selected strong{color:#fff;}

/* SUBF always visible */
.subf-label{display:flex;align-items:center;gap:10px;margin-bottom:10px;}
.si{width:32px;height:32px;border-radius:8px;background:rgba(108,99,255,0.12);border:1px solid rgba(108,99,255,0.3);display:flex;align-items:center;justify-content:center;font-size:15px;flex-shrink:0;}
.st1{font-size:13px;font-weight:600;color:#e2e8f0;}
.st2{font-size:11px;color:#818cf8;margin-top:1px;}
.stTextArea textarea{
    background:#0d0e1c!important;border:1.5px solid #2a2b55!important;
...
/* Extract button */
.stButton>button{
    width:100%!important;padding:16px!important;border:none!important;
...
.stButton>button:disabled{background:#1e2040!important;color:#4b5563!important;cursor:not-allowed!important;}

/* Security bar */
.sec-bar{background:#13142a;border:1px solid #2a2b55;border-radius:12px;padding:14px 18px;display:flex;align-items:center;justify-content:space-between;}
.sec-l{display:flex;align-items:center;gap:12px;}
.sec-icon{width:36px;height:36px;border-radius:9px;background:rgba(34,197,94,0.08);border:1px solid rgba(34,197,94,0.2);display:flex;align-items:center;justify-content:center;font-size:19px;flex-shrink:0;}
.sec-t1{font-size:13px;font-weight:600;color:#e2e8f0;}
.sec-t2{font-size:11px;color:#6b7280;margin-top:3px;}

/* Progress & Result */
.stProgress>div>div{background:linear-gradient(90deg,#6c63ff,#8b5cf6)!important;}
.result-box{background:rgba(108,99,255,0.07);border:1px solid rgba(108,99,255,0.25);border-radius:12px;padding:18px;margin-bottom:12px;}
.rb-t{font-size:14px;font-weight:700;color:#a78bfa;margin-bottom:12px;text-align:center;}
.rb-s{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;}
.rbs{background:rgba(0,0,0,0.3);border-radius:8px;padding:10px;text-align:center;}
.rbs-v{font-size:18px;font-weight:700;color:#fff;display:block;}
...
# SVG flags — consistent on ALL browsers (Chrome, Firefox, Safari, Edge)
FLAG_IN = """<svg class="flag-svg" viewBox="0 0 22 16" xmlns="http://www.w3.org/2000/svg">
  <rect width="22" height="16" fill="#FF9933"/>
  <rect y="5.33" width="22" height="5.34" fill="#fff"/>
  <rect y="10.67" width="22" height="5.33" fill="#138808"/>
  <circle cx="11" cy="8" r="2" fill="none" stroke="#000080" stroke-width="0.8"/>
</svg>"""

FLAG_FJ = """<svg class="flag-svg" viewBox="0 0 22 16" xmlns="http://www.w3.org/2000/svg">
  <rect width="22" height="16" fill="#68BFE5"/>
  <rect width="5" height="8" fill="#012169"/>
  <line x1="0" y1="0" x2="5" y2="8" stroke="#fff" stroke-width="1.5"/>
  <line x1="5" y1="0" x2="0" y2="8" stroke="#fff" stroke-width="1.5"/>
  <line x1="0" y1="0" x2="5" y2="8" stroke="#C8102E" stroke-width="0.8"/>
  <line x1="5" y1="0" x2="0" y2="8" stroke="#C8102E" stroke-width="0.8"/>
</svg>"""

FLAG_US = """<svg class="flag-svg" viewBox="0 0 22 16" xmlns="http://www.w3.org/2000/svg">
  <rect width="22" height="16" fill="#fff"/>
  <rect y="0" width="22" height="1.23" fill="#B22234"/>
  <rect y="2.46" width="22" height="1.23" fill="#B22234"/>
  <rect y="4.92" width="22" height="1.23" fill="#B22234"/>
  <rect y="7.38" width="22" height="1.23" fill="#B22234"/>
  <rect y="9.84" width="22" height="1.23" fill="#B22234"/>
  <rect y="12.3" width="22" height="1.23" fill="#B22234"/>
  <rect y="14.77" width="22" height="1.23" fill="#B22234"/>
...
# ── WELCOME ROW + CLOCKS with SVG flags ──
st.markdown(f"""
<div class="welcome-row">
  <div class="wl">
    <h2>Welcome Back Sublimation Team...! 👋</h2>
    ...
    <div class="feats">
      ...
      <div class="feat"><div class="fi">💻</div><div><div class="ft">No Manual Counting</div><div class="fs">We handle the rest</div></div></div>
      <div class="feat"><div class="fi">⚡</div><div><div class="ft">Instant Excel</div><div class="fs">Download in seconds</div></div></div>
    </div>
  </div>
  <div class="clocks">
    <div class="cc ist">
      <div class="cc-top">{FLAG_IN}<span class="cc-z">IST</span></div>
      <div class="cc-t" id="ist-t">--:--:--</div>
      <div class="cc-d" id="ist-d">--</div>
    </div>
    <div class="cc fjt">
      <div class="cc-top">{FLAG_FJ}<span class="cc-z">FJT</span></div>
      <div class="cc-t" id="fjt-t">--:--:--</div>
      <div class="cc-d" id="fjt-d">--</div>
    </div>
    <div class="cc est">
      <div class="cc-top">{FLAG_US}<span class="cc-z">EST</span></div>
...
# ── UPLOAD — single box with drop zone ──
st.markdown("""
<div class="upload-section">
  ...
  <div class="drop-overlay">
    <svg width="54" height="42" viewBox="0 0 54 42" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M41 35H13C7.477 35 3 30.523 3 25C3 19.72 6.99 15.4 12.15 14.79C13.43 9.8 17.93 6 23.25 6C30.16 6 35.75 11.59 35.75 18.5C35.75 18.74 35.74 18.97 35.72 19.2C39.13 19.63 41.75 22.53 41.75 26C41.75 26 43.5 26 43.5 28C43.5 31.87 41.37 35 41 35Z" fill="rgba(108,99,255,0.15)" stroke="#6c63ff" stroke-width="1.8"/>
      <path d="M27 28V20M27 20L22 25M27 20L32 25" stroke="#6c63ff" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/>
    </svg>
    <div class="drop-t1">Drag &amp; drop your PDF here</div>
    <div class="drop-t2">Or click the button below to browse files</div>
...
if uploaded_files:
    ...
    <div class="file-selected">
      ✅ &nbsp;<strong>{len(uploaded_files)} file(s) ready:</strong> {names}
    </div>
    """, unsafe_allow_html=True)

# ── SUBF ALWAYS VISIBLE ──
st.markdown("""
<div class="subf-label">
  <div class="si">#️⃣</div>
  <div>
    <div class="st1">SUBF Number Mapping <span style="font-size:11px;color:#6b7280;font-weight:400">(optional)</span></div>
    <div class="st2">Paste SUBF numbers to improve extraction accuracy</div>
  </div>
</div>
""", unsafe_allow_html=True)
...
subf_text = st.text_area(
    "subf",
    placeholder="SUBF 103566\t88251-24\nSUBF 103567\t88251-25",
    height=90,
